<template>
	<div>
		<h2 class="list-head">发起类型列表,共<span>{{page.total}}</span>条</h2>
			...
			<div class="addT-content">
				<el-table
				    ...
				    <el-table-column
				      ...
				      label="发起类型名称">
				      <template slot-scope="scope">
				      	<el-input rows=1 resize='none' type='textarea' v-model='scope.row.name' placeholder="请输入发起类型名称"></el-input>
				      </template>
				    </el-table-column>
				    <el-table-column
				      prop="remark"
		...
		methods:{
			...
			handleEdit(index, row) {
				if(!this.tableData[index].show){
	        		...
	        		this.$CPOST('/launchType/updateLaunchType',{
	        			'ltId':this.tableData[index].itId,
	        			'ltName':this.tableData[index].name,
			...
			handleDelete(index,row){
				this.$alert('确定要删除吗？', '', {
		          ...
		          callback: action => {
		          	if(action=='confirm'){
      		          	this.$CPOST('/launchType/delLaunchType',{
      						'ltId':this.tableData[index].itId
      					},(res)=>{
      						if(res.code==200){
			...
			add(){
				this.$router.push({path:'/index/addStarttype'});
			},
			searchL(){
				const loading = this.$loading(this.$store.state.loading);
				this.$CPOST('/launchType/findAllLaunchType',{
